...
All endpoints (except `/api/v1/user/exchangeToken` and `/api/v1/health`) require authentication via API key passed in the `X-API-Key` header.
...
2. The user exchanges the setup token for an API key via `POST /api/v1/user/exchangeToken`
...
*   **ACL Validation:** User must have appropriate permissions on the `.acl` item to submit ACL rules.
*   **Example Request:**
...
*   **Authentication:** Required (API key)
*   **Request:**
    *   JSON body with `user` (required) - ID of the user whose API keys to invalidate
*   **Response:**
...
*   **Authentication:** Required (API key)
*   **Request:**
    *   JSON body with `user` (required) - ID of the user to generate setup token for
*   **Response:**
