...
This document describes the v1 API endpoints for the `simple-sync` system.
...
Access to the ACL API endpoints is only allowed to users with the `admin_users` role.

## Users API

**Note:** Users API endpoints are not yet implemented in the current version.

The following endpoints are part of the Users API and are used for managing users. These endpoints use the same authentication mechanism as the regular user API. 
...
Access to the Users API endpoints is only allowed to users with the `admin_users` role.
