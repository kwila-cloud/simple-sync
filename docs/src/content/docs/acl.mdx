...
ACL rules are managed by submitting ACL events to the dedicated [`POST /api/v1/acl`](/simple-sync/api/v1#post-apiv1acl) endpoint. This endpoint requires API key authentication and automatically timestamps the events to prevent historical rule injection.

The ACL event format is:
```json
[{
  "user": "string",
  "item": "string",
