...
- Rule: user="*", item="*", action="*" → score: 0.5 + 0.5 + 0.5 = 1.5
- Rule: user="user123", item="*", action="*" → score: 7 + 0.5 + 0.5 = 8 (exact user match)
- Rule: user="*", item="task.*", action="*" → score: 0.5 + 5.5 + 0.5 = 6.5 (prefix item match)
- Rule: user="*", item="*", action="edit" → score: 0.5 + 0.5 + 4 = 5 (exact action match)

The rule with user="user123" wins with score 7.

For item "task.456" and action "edit", if no exact user match:

- Rule: user="*", item="task.*", action="*" → score: 0 + 6.5 + 4 = 10.5
...
If multiple rules have the same item, user, and action specificity, the most recently added rule (highest timestamp) takes precedence. If no rule matches, the default behavior (deny all actions) applies.
...
ACL rules are managed by submitting events to the POST /api/v1/events endpoint with `item` set to ".acl" and `action` set to "allow" or "deny". The payload must contain `user`, `item`, and `action` fields defining the rule. ACL events are validated against the current ACL before being added to the authoritative history; invalid ACL events are ignored.
